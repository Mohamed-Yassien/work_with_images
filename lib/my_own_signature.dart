...
class MyNewSignature extends StatefulWidget {
  /// constructor
  const MyNewSignature({
  ...
  @override
  State createState() => MyNewSignatureState();
}

/// signature widget state
class MyNewSignatureState extends State<MyNewSignature> {
                ...
                child: RepaintBoundary(
                  child: CustomPaint(
                    painter: _SignaturePainter(
                      widget.controller,
                      penWidth: widget.penW,
                      penColor: widget.penC,
                    ),
  ...
  @override
  void didUpdateWidget(covariant MyNewSignature oldWidget) {
  ...
  void _addPoint(PointerEvent event, PointType type) {
    ...
    // DIMENSIONS
    final double _maxSafeWidth = screenSize!.width;
    // final double _maxSafeWidth =
    //     maxWidth == double.infinity ? screenSize!.width : maxWidth;
    final double _maxSafeHeight = screenSize!.height;
...
class _SignaturePainter extends CustomPainter {
  _SignaturePainter(this._controller, {Color? penColor, double? penWidth})
      : _penStyle = Paint(),
        super(repaint: _controller) {
    _penStyle
      ..color = penColor ?? _controller.penColor
      ..strokeWidth = penWidth ?? _controller.penStrokeWidth
...
class SignatureController extends ValueNotifier<List<Point>> {
  ...
  /// Will return `null` if there are no points.
  Future<ui.Image?> toImage({int? width, int? height, Color? color}) async {
    if (isEmpty) {
      return null;
    }
    ...
    _SignaturePainter(this, penColor: color).paint(
      canvas,
      Size.infinite,
    );
  ...
  /// Will return `null` if there are no points.
  Future<Uint8List?> toPngBytes({int? height, int? width, Color? color}) async {
    if (kIsWeb) {
      return _toPngBytesForWeb(height: height, width: width, color: color);
    }
    final ui.Image? image =
        await toImage(height: height, width: width, color: color);
  ...
  /// Will return `null` if there are no points.
  Uint8List? _toPngBytesForWeb({int? height, int? width, Color? color}) {
    if (isEmpty) {
      return null;
    }
    ...
    final img.Color pColor = img.ColorRgb8(
      ...
      color?.green ?? penColor.green,
      color?.blue ?? penColor.blue,
    );
